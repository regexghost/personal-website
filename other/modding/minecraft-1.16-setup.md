+++
title = "A Minecraft Setup for 1.16"
description = "Minecraft Vanilla+ Mod Setup (1.16.5)"
+++

For years I've planned a grand Minecraft "forever world" but I always spend so much time planning it that I never actually play the game. This is a stripped down setup for 1.16 (because I never really played the nether update), not designed as a forever world setup, just a fun world to play around and chill in.

## Rules of Play

* Beds to save spawn point only, avoid sleeping to skip the night
* No industrial villager breeding/trade manipulation
* Don't rush to Elytra, use other transport
* No massive automated farms - small ones are okay
* Seed hunting can take 10 minutes max

## Mods

### Performance

* Better Beds: https://www.curseforge.com/minecraft/mc-mods/better-beds
* Enhanced Block Entities: https://modrinth.com/mod/ebe
* Entity Culling: https://modrinth.com/mod/entityculling
* Ferrite Core: https://modrinth.com/mod/ferrite-core
* Not Enough Crashes: https://modrinth.com/mod/notenoughcrashes
* Sodium: https://modrinth.com/mod/sodium
* Lithium: https://modrinth.com/mod/lithium

### Graphics

* Continuity: https://github.com/PlayReissLP/Continuity
* Falling Leaves: https://modrinth.com/mod/fallingleaves
* Distant Horizons: https://modrinth.com/mod/distanthorizons

### Sound

* Ambient Sounds: https://www.curseforge.com/minecraft/mc-mods/ambientsounds
* or Charmonium: https://www.curseforge.com/minecraft/mc-mods/charmonium
* Extra Sounds: https://www.curseforge.com/minecraft/mc-mods/extrasounds
* Cool Rain: https://modrinth.com/mod/coolrain

### UI

* Better F3: https://modrinth.com/mod/betterf3

### Util

* Mod Menu: https://modrinth.com/mod/modmenu
* Fabrishot: https://modrinth.com/mod/fabrishot
* Screenshot Viewer: https://modrinth.com/mod/screenshot-viewer
* Disable Custom Worlds Advice: https://www.curseforge.com/minecraft/mc-mods/fabric-disable-custom-worlds-advice
* Resource Pack Overrides: https://www.curseforge.com/minecraft/mc-mods/resource-pack-overrides

### Random

* Villager Names: https://www.curseforge.com/minecraft/mc-mods/villager-names
* Immersive Aircraft: https://www.curseforge.com/minecraft/mc-mods/immersive-aircraft
* Linkart: https://modrinth.com/mod/linkart
* Advancement Frames: https://www.curseforge.com/minecraft/mc-mods/advancement-frames
* Map Atlases: https://modrinth.com/mod/map-atlases
* Sword Blocking Mechanics: https://github.com/Fuzss/sword-blocking-mechanics

## Datapacks

### Vanilla Tweaks

#### Main

* Survival: Coordinates HUD
* Survival: Nether Portal Coords
* Survival: Durability Ping
* Mobs: Dragon Drops
* Mobs: More Mob Heads

#### Crafting

* Craftables: Craftable Name Tags

## Texture Packs

* Invisible Item Frames: https://modrinth.com/resourcepack/invisible-item-frames
* Simple 3D Chain: https://modrinth.com/resourcepack/simple-3d-chain/versions
* Crops 3D: https://www.curseforge.com/minecraft/texture-packs/crops-3d

### Vanilla Tweaks

* Aesthetic: Different Stems
* Aesthetic: Red Iron Golem Flowers
* Aesthetic: Bed Ironcs
* Aesthetic: Flint Tipped Arrows
* Peace and Quiet: Quieter Pistons
* Utility: Diminishing Tools
* Utility: Clear Banner Patterns
* Utility: Sticky Piston Sides
* Utility: Groovy Levers
* Utility: Brewing Guide + Dark UI
* Unobtrusive: Alternate Enchant Glint
* HUD: Rainbow Experience Bar
* GUI: Dark UI
* Fixes: Item Stitching Fix
* Fixes: Don't Eat the Bowl
* Fixes: Hoe Fix

## Other

* Set `doInsomnia` to false
* Set `doFireTick` to false
